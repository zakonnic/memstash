// Package gomemcache_adapter adapts the bradfitz memcached client (github.com/bradfitz/gomemcache) to the
...
// per-call deadline. Bound the latency with memcache.Client.Timeout instead.
package gomemcache_adapter
...
// BatchGet fetches all keys in one native GetMulti round trip. Raise memcache.Client.MaxIdleConns (the default is
// only 2) when large batches run concurrently. The context is ignored: the client has no context support.
