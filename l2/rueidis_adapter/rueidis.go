...
func IsOrderedMgetAvailable(client rueidislib.Client) bool {
...
func NewStringCache[K comparable](client rueidislib.Client, opts ...memstash.Option) (*memstash.Cache[K, string], error) {
...
// replies (see l2.WithOrderedMget) and the rueidis MGet helper otherwise, a DoMulti pipeline of GETs above it.
...
func (c *Cache[K, V]) batchGetOrdered(ctx context.Context, keys []K, storageKeys []string, found memstash.List[K, V]) (memstash.List[K, V], error) {
	...
	if len(replies) != len(keys) {
		return found, fmt.Errorf("memstash/l2/rueidis: MGET returned %d replies for %d keys", len(replies), len(keys))
	}
	for i := range replies {
		if replies[i].IsNil() {
			continue
...
// rueidis pipelines every command onto shared connections and only moves a pipeline off them past
...
func (c *Cache[K, V]) doMultiLarge(ctx context.Context, cmds []rueidislib.Completed) error {
	if !c.singleNode {
...
func (c *Cache[K, V]) doMultiLargeResults(ctx context.Context, cmds []rueidislib.Completed) []rueidislib.RedisResult {
	if !c.singleNode {
