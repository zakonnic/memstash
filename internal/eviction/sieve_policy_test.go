...
func TestSieveEvictionOrder(t *testing.T) {
	...
	for i := 0; i < 3; i++ {
		...
		victims = append(victims, pool.items.At(idx).Entry().Key)
		pool.release(idx)
	}
...
func TestSieveRetainedStayInPlace(t *testing.T) {
	...
	idx, ok := p.Evict(0)
	require.True(t, ok)
	assert.Equal(t, "cold1", pool.items.At(idx).Entry().Key, "the visited oldest item must survive the scan")
	pool.release(idx)
	...
	idx, ok = p.Evict(0)
	require.True(t, ok)
	assert.Equal(t, "cold2", pool.items.At(idx).Entry().Key, "a retained item must not be re-queued behind newcomers")
	pool.release(idx)

	assert.Equal(t, 1, p.Len())
	live := 0
	p.Rebuild(func(old uint32) (uint32, bool) {
		assert.Equal(t, "hot", pool.items.At(old).Entry().Key)
		live++
		return old, true
	})
	assert.Equal(t, 1, live, "Rebuild must visit each queued node exactly once")
}
...
func TestSieveSweep(t *testing.T) {
	...
	idx, ok := p.Evict(0)
	require.True(t, ok)
	require.Equal(t, "k1", pool.items.At(idx).Entry().Key)
	pool.release(idx)

	for _, i := range []int{2, 4} {
		pool.items.At(indices[i]).Kill()
	}
	released := map[string]bool{}
	p.Sweep(func(idx uint32) {
		released[pool.items.At(idx).Entry().Key] = true // Entry is still readable: Release happens in the callback's caller
		pool.release(idx)
	})
	...
	for i := 0; i < 3; i++ {
		...
		victims[pool.items.At(idx).Entry().Key] = true
		pool.release(idx)
	}
