...
type Policy[K comparable, V any] interface {
	...
	// caller then removes the corresponding table slot, subtracts its weight, and returns the record to the pool.
	...
	// Range calls f for every queued node (dead ones included) without disturbing the queues. Every live record is
	// visited exactly once - the cache uses this to rebuild a shard's slot table.
