...
func TestS3FIFOPolicyBytes(t *testing.T) {
	pool := newArena()
	p := NewS3FIFO(&pool.items, 1000, 100)
...
func TestS3FIFOGhostPromotion(t *testing.T) {
	...
	idx, ok := p.Evict(0)
	require.True(t, ok)
	require.Equal(t, "victim", pool.items.At(idx).Entry().Key, "the untouched head of small must be the victim")
	pool.release(idx)
